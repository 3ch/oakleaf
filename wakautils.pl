...
sub do_spans($@)
{
	...
	return join "<br />",map
	{
		...
		# Wortfilter
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (Krautchan) (?![0-9a-zA-Z\*_]) }{Bielefeld}gx;
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (Tim\ Kaiser) (?![0-9a-zA-Z\*_]) }{tka}gx;
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (Sandneger) (?![0-9a-zA-Z\*_]) }{ICH BIN SO D&auml;MLICH DAS GEHT GAR NICHT}gx;
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (\:erdbeere\:) (?![0-9a-zA-Z\*_]) }{<img src="http://3-ch.org/static/erdbeere.gif" alt="erdbeere" style="vertical-align: bottom;" />}gx;
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (\:tanz\:) (?![0-9a-zA-Z\*_]) }{<img src="http://3-ch.org/static/tanz.gif" alt="imanouchi" style="vertical-align: bottom;" />}gx;
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (\:ernstchan\:) (?![0-9a-zA-Z\*_]) }{<img src="http://3-ch.org/static/ec.jpg" alt="ec" style="vertical-align: bottom;" />}gx;
		$line=~s{ (?<![0-9a-zA-Z\*_\x80-\x9f\xe0-\xfc]) (\:3ch\:) (?![0-9a-zA-Z\*_]) }{<img src="http://3-ch.org/static/3ch.jpg" alt="3ch" style="vertical-align: bottom;" />}gx;
...
sub make_date($$;@)
{
	...
	if($style eq "2ch")
	{
		...
		$ltime[3],$ltime[4]+1,$ltime[5]+1900,$ltime[2],$ltime[1]);
	}
	elsif($style eq "futaba")
	{
		my @ltime=localtime($time);
