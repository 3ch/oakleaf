...
# Page look
use constant TITLE => 'B&uuml;ro f&uuml;r VIP-Qualit&auml;t @3ch';	# Name of this image board
...
use constant THREADS_LISTED => 200;				# Number of threads in the thread list
#use constant REPLIES_PER_THREAD => 10;			# Replies shown
use constant S_ANONAME => 'VIPPER';			# Defines what to print if there is no text entered in the name field
use constant DEFAULT_STYLE => 'VIPPER';		# Default CSS style title
...
#use constant AUTOCLOSE_DAYS => ;				# Maximum number of days with no activity before a thread closes. 0 to disable.
...
use constant ALLOW_ADMIN_EDIT => 1;			# Allow editing of include files and spam.txt from admin.pl.
...
use constant DISPLAY_ID => 'day thread board';# How to display user IDs (0 or '': don't display,
...
use constant EMAIL_ID => 'sage!';				# Replace the ID with this string when the ID is hidden (DISPLAY_ID contains 'sage' or 'link').
...
use constant ALTERNATE_REDIRECT => 1;			# Use alternate redirect method. (Javascript/meta-refresh instead of HTTP forwards.)
...
use constant STYLE_COOKIE => '3chstyle';	# Cookie name for the style selector.
...
use constant KEEP_MAINPAGE_NEWLINES => 1;		# Don't strip whitespace from main page (needed for Google ads to work, 0:no, 1:yes)
...
use constant ADMIN_MASK_IPS => 1;					# Mask poster IP addresses in the admin script (0: no, 1: yes)
