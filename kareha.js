...
function reload_captcha() {
	...
	document.cookie="captchakey=; path=/; expires=" + cookie_date.toGMTString();
	document.cookie="captchakey=; path=/" + match[1] + "/; expires=" + cookie_date.toGMTString();
	var elements = document.getElementsByTagName("img");
	var i;
	for (i = 0, j = 0; i < elements.length; i++) {
		if ( elements[i].className == "postcaptcha") {
			elements[i].src = '';
			elements[i].src = captcha_src + '&force_refresh=' + cookie_date.toGMTString();
		}
	}
	
	elements = document.getElementsByTagName("a");
	for (i = 0, j = 0; i < elements.length; i++) {
		if ( elements[i].className == "reloadlink") {
			elements[i].firstChild.nodeValue='Captcha neu laden (10 Sekunden warten)';
			elements[i].href="javascript:;";
			elements[i].className="reloadlink_d";
		}
	}
	
	setTimeout( 'reenable_reload()', 10000 );
}

function reenable_reload() {
	var elements = document.getElementsByTagName("a");
	for (i = 0, j = 0; i < elements.length; i++) {
		if ( elements[i].className == "reloadlink_d") {
			elements[i].firstChild.nodeValue='Captcha neu laden';
			elements[i].href="javascript:reload_captcha();";
			elements[i].className="reloadlink";		
		}
	}
}
var captcha_src;
function add_reload_links() {
	var elements = document.getElementsByTagName("img");
	var i;
	for (i = 0, j = 0; i < elements.length; i++) {
		if ( elements[i].className == "postcaptcha") {
			captcha_src = elements[i].src;
			var small = document.createElement('small');
			var link = document.createElement('a');
			link.href="javascript:;";
			link.className="reloadlink_d";
			link.appendChild( document.createTextNode("Captcha neu laden (10 Sekunden warten)") );
			small.appendChild( link );
			elements[i].parentNode.appendChild( small );
		}
	}
	setTimeout( 'reenable_reload()', 10000 );
}